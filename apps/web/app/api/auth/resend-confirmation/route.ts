...
export async function POST(req: Request) {
  try {
    ...
    const ok = await rateLimit(`auth:resend:${ip}`, 5, 60);
    ...
    const { error } = await supabase.auth.resend({
      ...
      options: { emailRedirectTo: `${getAppBaseUrl(req)}/auth/callback` }
    });
    ...
    if (error) {
      ...
      return NextResponse.json(
        { error: `No se pudo reenviar el correo: ${error.message}` },
        { status: 400 }
      );
    }

    return NextResponse.json({ ok: true, message: 'Correo reenviado' });
  } catch (error: any) {
    console.error('resend-confirmation-fatal', error?.message || error);
