...
import { getRiskThreadIds } from '@/lib/host-messaging-config';
...
export default async function ClientMessagesPage({ searchParams }: { searchParams: Record<string, string | string[] | undefined> }) {
  ...
  const inquiryActivityIds = Array.from(
    new Set(
      threads
        .map((thread) =>
          thread.subject?.startsWith('ACTIVITY:') ? thread.subject.replace('ACTIVITY:', '').trim() : null
        )
        .filter((id): id is string => Boolean(id))
    )
  );
        ...
        <aside className="surface-card hidden h-full min-h-0 flex-col overflow-y-auto lg:flex">
          ...
          {selectedIsRisk ? (
            <div className="mt-3 rounded-2xl border border-amber-300 bg-amber-50 p-3 text-xs text-amber-800">
              Conversacion marcada por riesgo de pago por fuera. Mantene el pago dentro de Hostea.
            </div>
          ) : null}
          {selectedThread?.reservation ? (
